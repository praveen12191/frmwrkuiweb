...
const DataEntry = () => {
    ...
    useEffect(() => {
        const url = 'http://localhost:8000/columnDetail';
    ...
    //     setIsLoading(true); 
    //     axios.post('http://localhost:8000/columnName', { tablename: tableName})
    //         .then(response => {
    ...
    const handleTableHover = (tableName, columnName) => {
        ...
        axios.post('http://localhost:8000/columnDetails', { 
            tablename: tableName, 
            ...
            <form onSubmit={handleSubmit}>
                {Object.keys(columnDetails).map((tableName, index) => (
                    <div className="help" key={index} style={{ marginBottom: '40px' }}>
                        <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>{tableName.toUpperCase()}</h2>
                        <div 
                            style={{ 
                                display: 'flex', 
    ...
    <div key={colIndex} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '150px' }}>
        <label>{columnName}</label>
        <Autocomplete
            value={formData[tableName][columnName] || ''}
            onChange={(event, newValue) => handleInputChange(event, tableName, columnName, newValue)}
            onInputChange={(event, newInputValue) => handleInputChange(event, tableName, columnName, newInputValue)}    
